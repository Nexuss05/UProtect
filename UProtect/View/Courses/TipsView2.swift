...
struct TipsView2: View {
    ...
    @State private var showEndMessage = false // State to control visibility of end message
        ...
        private func checkDailyLimit() {
            let lastMidnight = Calendar.current.startOfDay(for: Date())
            let nextMidnight = lastMidnight.addingTimeInterval(24 * 60 * 60)
            let now = Date()
            
            // Reset viewed cards at midnight
            if now >= nextMidnight {
                UserDefaults.standard.removeObject(forKey: "ViewedCards")
                canInteract = true
                showEndMessage = false
            } else {
                // Check if daily limit has been reached
                let viewedCards = UserDefaults.standard.array(forKey: "ViewedCards") as? [Int] ?? []
                if viewedCards.count >= 3 {
                    canInteract = false
                    showEndMessage = true
                }
            }
        ...
        // Function to mark a card as viewed for the day
        private func markCardAsViewed(_ cardId: Int) {
            var viewedCards = UserDefaults.standard.array(forKey: "ViewedCards") as? [Int] ?? []
            viewedCards.append(cardId)
            UserDefaults.standard.set(viewedCards, forKey: "ViewedCards")
    ...
    var body: some View {
        ZStack {
            ...
            ZStack {
                ForEach(tips.prefix(3).reversed()) { tip in // Limit to display only 3 cards
                    VStack {
                        HStack {
                            Text("Tips & Tricks")
                                .font(.largeTitle)
                                ...
                                .onEnded({ value in
                                    withAnimation {
                                        if tip.id == scrolled {
                                            ...
                                            // Check if scrolled to the last card
                                            if tip.id == tips.count - 1 {
                                                showEndMessage = true
                                            }
                                            
                                            // Mark current card as viewed for the day
                                            markCardAsViewed(tip.id)
                                        }
                                    }
                                }) : nil
                        )
                ...
                // End message when scrolled to the last card or daily limit is reached
                if showEndMessage || !canInteract {
                    VStack {
                        Text("To see more cards, wait until tomorrow")
                            .font(.title)
                            .fontWeight(.bold)
        ...
        .onAppear {
            // Check if daily limit has been reached when view appears
            checkDailyLimit()
        }
    }
}
